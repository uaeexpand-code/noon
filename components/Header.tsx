...
export const Header: React.FC<HeaderProps> = ({ currentDate, setCurrentDate, onOpenSettings, onDiscoverEvents, isDiscovering, viewMode, setViewMode, onSendSummary, isSendingSummary, discordWebhookUrl, onOpenChat }) => {
  ...
  return (
    <header className="flex flex-wrap items-center justify-between gap-4 mb-4 sm:mb-6 p-4 rounded-lg bg-gray-800/50 backdrop-blur-sm">
        <div className="flex items-center space-x-2">
            <h1 className="text-xl sm:text-2xl font-bold text-white hidden sm:block">Seller's Calendar</h1>
        ...
        <div className="flex items-center flex-grow justify-center space-x-2 sm:space-x-4">
            <button
                ...
                className="px-3 py-2 text-sm font-medium text-gray-200 bg-gray-700 rounded-md hover:bg-cyan-500 hover:text-white transition-colors duration-200"
            >
                Today
            </button>
            <div className="flex items-center">
                <button onClick={() => handleNavigation('prev')} className="p-2 text-gray-400 hover:text-white rounded-full hover:bg-gray-700 transition-colors duration-200" aria-label="Previous period">
                    <ChevronLeftIcon />
                </button>
                <h2 className="w-32 sm:w-48 text-center text-sm sm:text-xl font-semibold text-white">
                    {getTitle(currentDate, viewMode)}
                </h2>
                <button onClick={() => handleNavigation('next')} className="p-2 text-gray-400 hover:text-white rounded-full hover:bg-gray-700 transition-colors duration-200" aria-label="Next period">
                    <ChevronRightIcon />
                </button>
            </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
            <div className="hidden sm:flex items-center bg-gray-700 rounded-md p-0.5">
                 {views.map(view => (
                     <button key={view} onClick={() => setViewMode(view)} className={`px-2 py-1 text-sm rounded-md capitalize transition-colors duration-200 ${viewMode === view ? 'bg-cyan-600 text-white' : 'text-gray-300 hover:bg-gray-600'}`}>
                         {view}
                     </button>
                 ))}
            </div>
             <button
              ...
              className="p-2 text-gray-400 hover:text-white rounded-full hover:bg-gray-700 transition-colors duration-200 disabled:cursor-not-allowed disabled:text-gray-600"
            ...
            <button
              ...
              className="p-2 text-gray-400 hover:text-white rounded-full hover:bg-gray-700 transition-colors duration-200 disabled:cursor-not-allowed"
